...
export default function WorkspaceGuests() {
  ...
  const { guestBoardCounts, singleBoardGuestCount } = useMemo(() => {
    if (!workspace?.boards) {
      return { guestBoardCounts: new Map<string, number>(), singleBoardGuestCount: 0 }
    }
    ...
    return { guestBoardCounts: boardCounts, singleBoardGuestCount: singleBoardCount }
                ...
                >
                  ...
                  <Stack direction='row' alignItems='center' gap={1} flexWrap='wrap'>
                    <Button
                      size='small'
                      variant='outlined'
                      disabled={guestBoardCounts.get(guest._id) === 0}
                      sx={{ borderRadius: 1, textTransform: 'none', minWidth: 120 }}
                    >
                      View boards ({guestBoardCounts.get(guest._id) || 0})
                    </Button>
                    <Button
                      size='small'
                      variant='outlined'
